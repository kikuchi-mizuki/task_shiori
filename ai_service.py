    ...
    def extract_dates_and_times(self, text):
        ...
        try:
            ...
            system_prompt = (
                ...
                "20. **最小空き時間（例：○時間空いているところ、○分以上の空き時間）が指定されている場合は、minimum_duration_minutesフィールドに抽出してください。時間単位（時間、h）と分単位（分、min）の両方に対応してください。**\n"
                "    - 例：「今週2時間空いているところ」→ minimum_duration_minutes: 120\n"
                "    - 例：「明日3時間以上の空き時間」→ minimum_duration_minutes: 180\n"
                "    - 例：「来週90分空いているところ」→ minimum_duration_minutes: 90\n"
                "\n"
                "【出力例】\n"
                "空き時間確認の場合:\n"
